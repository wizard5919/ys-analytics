...
CONFIG = {
    ...
    'FINNHUB_API_KEY': '', # Added for sentiment
    'MAX_RETRIES': 5,
    'RETRY_DELAY': 2,
    'DATA_TIMEOUT': 30,
    'MIN_DATA_POINTS': 50,
...
SIGNAL_THRESHOLDS = {
    'call': {
        ...
        'condition_weights': {
            ...
            'sentiment': 0.05 # Added sentiment weight
        }
    },
    'put': {
        'delta_base': -0.5,
...
def calculate_dynamic_sensitivity(data: pd.DataFrame, base_sensitivity: float) -> float:
    ...
    try:
        ...
        if 'High' in data.columns and 'Low' in data.columns and 'Close' in data.columns:
            ...
            atr = true_range.rolling(window=min(14, len(data))).mean().mean().iloc[-1]
          
            if not pd.isna(atr) and atr > 0:
                # Scale sensitivity based on ATR relative to price
                volatility_ratio = abs(atr) / current_price
...
def calculate_support_resistance_enhanced(data: pd.DataFrame, timeframe: str, current_price: float) -> dict:
    """
    Enhanced support/resistance calculation with proper level alignment
    """
...
def calculate_remaining_trading_hours() -> float:
    ...
    try:
        ...
        return 0.0
# UPDATED: Enhanced price fetching with multi-source fallback using async
async def async_get_current_price(ticker: str) -> float:
    """Async get real-time price from multiple free sources"""
    ...
    # Sync fallback to Yahoo Finance
    try:
...
def calculate_volume_averages(df: pd.DataFrame) -> pd.DataFrame:
    ...
    try:
        ...
        # Fill any any remaining NaN values with overall average
...
def generate_enhanced_signal(option: pd.Series, side: str, stock_df: pd.DataFrame, is_0dte: bool) -> Dict:
    ...
    try:
        ...
        explanations.append({
            ...
            'score': sr_score,
            'explanation': f"Strike near {'support' if side == 'call' else 'resistance'} (improves breakout odds)."
        })
...
with st.sidebar:
    ...
    with st.container():
        ...
        if st.session_state.get('last_refresh'):
            ...
            st.caption(f"**Cache**: {time_since}s ago ago")
            ...
            with st.expander("🎯 Trading Context", expanded=True):
                ...
                **💡 Best Practices:**
                ...
                if is_premarket():
                    ...
                    st.success("🟢 **MARKET OPEN**: Real-time data available")
...
if st.session_state.get('auto_refresh_enabled', False) and ticker:
    ...
    if elapsed > min_interval:
        ...
        st.rerun()
# Comment for production authentication
# In production, use st.secrets for API keys or integrate with external auth service like OAuth
# =============================
# UNIT TESTS
# =============================
...
# Run tests separately with: python -m unittest discover
